...
- 카드별 독립 정답 확인과 `몰랐음 / 애매함 / 알았음` 판정
- 숙련 단계와 성공 연속 횟수에 따른 로컬 간격 반복
...
모든 핵심 조작은 키보드와 스크린 리더 사용을 고려합니다. 카드에서 `Space`로 정답을 열고, `1 / 2 / 3`으로 판정하며, `P`로 음성을 재생하고, `R`로 다시 가릴 수 있습니다. `Esc`는 열린 상세 패널을 닫습니다. 숙련 상태는 색뿐 아니라 모양과 텍스트로도 전달합니다.
